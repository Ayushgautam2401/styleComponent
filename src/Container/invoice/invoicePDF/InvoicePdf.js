...
const MyDocument = ({ invoice }) => {
  ...
  const { invoiceDate, invoiceNumber, firmDetail, invoiceRaisedBy, totalAmount, currencyType } = invoice;
    ...
    <Document>
      <Page size="A4" style={[styles.flex, styles.flexColumn, styles.page]}>
        <View style={[styles.flex, styles.flexAlignCenter, styles.marginBottom10]}>
          ...
          <Text style={styles.fontSm}>{new Date(invoiceDate).getDate()}</Text>
          {/* <Text style={styles.fontSm}>{new Date(`${invoiceDate.getDate()}/${invoiceDate.getMonth() + 1}/${invoiceDate.getFullYear()}`)}</Text> */}
        </View>
        {renderFirmDetail()}
        ...
        <View style={[styles.flex, styles.flexRow, styles.flexJustifyBetween, styles.marginBottom10]}>
          ...
          <View style={[styles.width50, styles.borderBox]}>
            ...
            {renderTableDetail(["Invoice Date", new Date(invoiceDate).getDate()], [50, 50], [])}
        ...
        <Text style={[styles.fontmd]}>{`${invoiceRaisedBy} (${firmDetail.name})`}</Text>
      </Page>
    </Document>
  )
};
